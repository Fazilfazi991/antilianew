...
export function Hero() {
  ...
  return (
    <section className="relative min-h-[calc(100dvh-76px)] w-full overflow-hidden">
      <video
        autoPlay
        muted
        loop
      ...
      <div className="relative z-10 flex min-h-[calc(100dvh-76px)] flex-col items-center justify-center px-5 pb-7 pt-12 text-center md:px-12 md:pt-14">
        ...
        <h1 className="mb-3 flex flex-wrap justify-center gap-x-3 font-display-xl text-display-xl text-white">
          {words.map((word, i) => (
            <motion.span
              key={word + i}
        ...
        <motion.p
          className="mb-5 max-w-xl font-body-md text-body-md text-white/75"
          initial={reduced ? { opacity: 0 } : { opacity: 0, y: 12 }}
          animate={{ opacity: 1, y: 0 }}
        ...
        <motion.form
          ...
          className="mb-4 w-full max-w-[720px]"
          initial={reduced ? { opacity: 0 } : { opacity: 0, y: 16 }}
          animate={{ opacity: 1, y: 0 }}
          transition={{ delay: words.length * 0.1 + 0.2, duration: 0.6 }}
        >
          <div className="overflow-visible rounded-[18px] border border-white/30 bg-[#17191f]/80 p-3 text-left shadow-[0_20px_50px_rgba(0,0,0,0.34)] backdrop-blur-xl sm:p-4">
            <p className="mb-2 flex items-center gap-2 font-body-md text-[14px] font-medium text-[#d9b780] sm:text-[15px]">
              <span className="material-symbols-outlined text-[#d9b780]" style={{ fontSize: 22 }}>flare</span>
              Start with Buy or Rent
            </p>
            <div className="space-y-2">
              <div className="grid max-w-[380px] grid-cols-2 rounded-full border border-white/30 bg-black/20 p-1">
                {TRANSACTION_TABS.map(tab => {
                  ...
                  return (
                    <button
                      ...
                      aria-pressed={active}
                      className={`flex h-10 items-center justify-center gap-2 rounded-full px-4 font-body-md text-[15px] font-semibold transition-all focus:outline-none focus:ring-2 focus:ring-[#d9b780] sm:text-[16px] ${
                        active
                    ...
                    {SEGMENT_TABS.map(tab => {
                      ...
                      return (
                        <button
                          ...
                          aria-pressed={active}
                          className={`flex h-10 items-center justify-center gap-2 rounded-lg border px-3 font-body-md text-[14px] font-semibold transition-colors focus:outline-none focus:ring-2 focus:ring-inset focus:ring-[#d9b780] sm:text-[15px] ${
                            active
            ...
            <div className="grid gap-2 sm:grid-cols-[1fr_160px]">
              ...
              <button
                ...
                className="flex h-11 items-center justify-center gap-2 rounded-lg bg-gradient-to-br from-[#ddc08e] to-[#a98451] px-6 font-body-md text-[16px] font-semibold text-white shadow-[0_8px_20px_rgba(191,151,92,0.25)] transition-transform hover:-translate-y-0.5 disabled:cursor-not-allowed disabled:bg-none disabled:bg-white/15 disabled:text-white/45 disabled:shadow-none"
              >
                <Search aria-hidden="true" className="size-5" />
                Search
              </button>
            </div>
            {!canSearch && !locationOpen && <p id="search-requirements" className="pt-2 font-body-md text-[12px] text-white/55">Choose Buy or Rent, then select a property category to continue.</p>}
          </div>
        </motion.form>
        ...
        >
          <motion.div whileHover={reduced ? {} : { scale: 1.03 }} whileTap={reduced ? {} : { scale: 0.97 }}>
            <Link
              to="/properties"
              className="inline-block border border-white/50 px-5 py-2.5 font-label-caps text-label-caps tracking-[0.1em] text-white uppercase transition-all duration-300 hover:border-white hover:bg-white/10"
            >
              Explore Properties
            </Link>
          </motion.div>
          <motion.div whileHover={reduced ? {} : { scale: 1.03 }} whileTap={reduced ? {} : { scale: 0.97 }}>
            <Link
              to="/portal/login"
              className="inline-block bg-[#A68966] px-5 py-2.5 font-label-caps text-label-caps tracking-[0.1em] text-white uppercase transition-colors duration-300 hover:bg-[#8a6e4e]"
            >
              List Your Property
            </Link>
          </motion.div>
