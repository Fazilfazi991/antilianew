...
import { LayoutDashboard, ListFilter, PlusCircle, User, LogOut, Loader2 } from 'lucide-react';
import { useAuth } from '@/hooks/useAuth';
import { agentSignOut, fetchProfile } from '@/lib/queries/agent';
import type { Profile } from '@/lib/types';
import { DashboardMobileNav } from '@/components/DashboardMobileNav';

const NAV = [
  { to: '/agent',              label: 'Dashboard',       icon: LayoutDashboard, exact: true },
  { to: '/agent/listings',     label: 'My Listings',     icon: ListFilter,      exact: true },
  { to: '/agent/listings/new', label: 'Submit Property', icon: PlusCircle,      exact: false },
  { to: '/agent/profile',      label: 'My Profile',      icon: User,            exact: true },
];
  ...
  return (
    <div className="min-h-screen flex flex-col md:flex-row">
      <DashboardMobileNav
        ...
        profile={profile ? { name: profile.full_name || session?.user.email || 'Agent', subtitle: profile.position ?? undefined } : null}
      />
          ...
          <div className="mb-6 px-2 pb-6 border-b border-white/10">
            <div className="flex items-center gap-2.5">
              ...
              <div className="min-w-0">
                ...
                {profile.position && (
                  <p className="font-label-caps text-label-caps text-white/35 truncate uppercase tracking-[0.04em] text-[10px]">
                    {profile.position}
                  </p>
                )}
              </div>
