...
export function AgentListingFormPage() {
  ...
  async function handleImageUpload(files: FileList | null) {
    ...
    setUploading(true);
    setError('');
    const slug = 'agent-' + (session?.user.id?.slice(0, 8) ?? 'user');
    try {
    ...
    <div className="px-5 py-6 md:px-10 md:py-10 max-w-4xl">
      ...
      <div className="mb-6 border border-surface-variant p-4 bg-surface-container-low">
        <p className="font-label-caps text-label-caps text-on-surface-variant uppercase tracking-[0.08em]">
          Your listing will be reviewed by the admin team before going live. Contact details are managed by Antilia.
        </p>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        <Section title="Basic Info">
          <Field label="Title *">
            <input className={inputClass} value={form.title} onChange={e => set('title', e.target.value)} required placeholder="e.g. Luxury Apartment in The Pearl" />
          </Field>
