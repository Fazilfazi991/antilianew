...
import { ArrowLeft, Loader2, Upload, X, Star, Info } from 'lucide-react';
import {
  createMarketingProperty,
  updateMarketingProperty,
  fetchMarketingPropertyById,
  uploadMarketingPropertyImage,
...
export function MarketingPropertyFormPage() {
  ...
  async function handleImageUpload(files: FileList | null) {
    ...
    try {
      ...
      for (const file of Array.from(files)) {
        const url = await uploadMarketingPropertyImage(file, form.slug);
  ...
  async function handleSubmit(e: React.SyntheticEvent<HTMLFormElement>) {
    ...
    try {
      if (isEdit && id) {
        await updateMarketingProperty(id, form);
      } else {
        await createMarketingProperty(form, session.user.id);
      }
      navigate('/marketing/properties');
    } catch (e: unknown) {
  ...
  return (
    <div className="px-5 py-6 md:px-10 md:py-10 max-w-4xl">
      ...
      {!isEdit && (
        <div className="flex items-start gap-3 border border-amber-200 bg-amber-50 p-4 mb-6">
          <Info className="size-4 text-amber-600 mt-0.5 shrink-0" />
          <p className="font-body-md text-body-md text-amber-700">
            This listing will be submitted for admin review and won't go live until approved.
          </p>
        </div>
      )}
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
            ...
            <Field label="Area / Neighbourhood *">
              <input
                ...
                required
                placeholder="e.g. West Bay, The Pearl, Lusail Marina"
              />
            </Field>
          </div>
        </Section>
